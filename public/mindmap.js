...
function setup() {
    ...
    center.transform( "t" + ( centerX - 16 ) + "," + ( centerY - 16 ) ).transform( "...s4,4" );
    center.data( "position", centerPosition );
...
function addCloud( cloud, center, disableClick ) {
  var id = cloud.id,
    ...
    x_pos = center.x + ( Math.cos(rotation * (Math.PI/180)) * distance),
    y_pos = center.y - ( Math.sin(rotation * (Math.PI/180)) * distance),
    position = {x: x_pos, y: y_pos},
    node,
    text;

  node = paper.path( filledCloud )
          .attr({
            fill: "#333",
            stroke: "none"
          })
          .transform( "t" + x_pos + "," + y_pos )
          .transform( "...s3,3" )
          .data( "position", position);

  node.node.draggable = false;
  text = paper.text( x_pos + 16, y_pos + (16 * 3) / 2, title ).attr({ fill: "#fff" });
